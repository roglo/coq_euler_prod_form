...
Theorem summation_split_last : ∀ b k g,
  b ≤ k
  → ∑ (i = b, k), g i = ∑ (i = S b, k), g (i - 1)%nat + g k.
Proof.
intros * Hbk.
...
intros.
progress unfold iter_seq, iter_list.
progress replace (S (s + k) - (s + b)) with (S k - b) by flia.
remember (S k - b)%nat as len; clear Heqlen.
clear k.
revert b d.
induction len; intros; [ easy | ].
...
Theorem summation_shift : ∀ s b g k,
  s ≤ b ≤ k
...
Theorem summation_rshift : ∀ s b e f,
...
Theorem summation_only_one : ∀ g n, ∑ (i = n, n), g i = g n.
Proof.
intros g n.
apply iter_seq_only_one, Nat.add_0_l.
Qed.
...
Theorem Nat_div_lt_mul : ∀ a b c, b ≠ 0 → a / b < c → a < b * c.
Proof.
intros * Hbz Habc.
apply (Nat.mul_le_mono_l _ _ b) in Habc.
apply (Nat.lt_le_trans _ (b * S (a / b))); [ | easy ].
...
destruct (Nat.eq_dec n 0) as [Hnz| Hnz]. {
  subst n; rewrite Nat.gcd_0_r in Hg; subst c.
  now do 2 rewrite Nat.mul_1_r in Hab.
}
