...
progress unfold coprimes in Hap.
...
Theorem Nat_eq_gcd_1 : ∀ a b, Nat.gcd a b = 1 → Nat.divide b (a - 1).
Proof.
intros * Hab.
Search Nat.gcd.
...
rewrite <- Nat_gcd_sub_diag_l in Hab.
...
...
Search (_ *  (_ / _ )).
...
  rewrite Nat.gcd_comm in Hap.
  rewrite <- Nat.Lcm0.gcd_mod in Hap.
  specialize (Nat.Div0.mod_eq a 2) as H1.
Search (Nat.gcd _ _ = 1).
Search (Nat.gcd _ _ = Nat.gcd _ _).
...
  specialize (Nat.div_mod a 2 (Nat.neq_succ_0 _)) as H1.
Search (_ * (_ / _)).
...

  specialize (Nat.mod_upper_bound 2 (S a) (Nat.neq_succ_0 _)) as H1.
  remember (a mod 2) as am2 eqn:Ham2; symmetry in Ham2.
  destruct am2; [ easy | ].
  destruct am2; cycle 1. {
    destruct a; [ easy | ].
    flia Ham2 H1.
  }
...
...
erewrite summation_eq_compat; cycle 1. {
  ...
  replace (2 * i * a / p) with (if h <? i * a then 1 else 0); cycle 1. {
    ...
    destruct b. {    
      apply Nat.ltb_lt in Hb.
      specialize (Nat.div_mod (i * a) h) as H1.
...
}
